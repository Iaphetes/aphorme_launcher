...
use iced::{
    event, executor, keyboard, subscription, theme, window, Alignment, Background, Color, Event,
    Subscription,
};
...
impl Application for ScrollableDemo {
    ...
    fn update(&mut self, message: Message) -> Command<Message> {
        match message {
            Message::Scrolled(offset) => {
                ...
                // snap_to(self.scrollable_id.clone(), self.current_scroll_offset);
                Command::none()
            }
            Message::Scroll(offset) => {
                // self.current_scroll_offset.x += offset.x;
                let move_amount = 1.0 / self.application_manager.matches.len() as f32;
                ...
                scrollable::snap_to(self.scrollable_id.clone(), self.current_scroll_offset)
            }
        }
    }
    fn view(&self) -> Element<Message> {
        ...
        let scrollable_content = Element::from(
            scrollable(
                column
                    .width(Length::Fill)
        ...
        let content = column![scrollable_content,]
            .width(Length::Fill)
            .height(Length::Fill)
            .align_items(Alignment::Center)
            .spacing(10)
