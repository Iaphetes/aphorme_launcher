...
use eframe::{
    egui::{
        load::SizedTexture, Align, CentralPanel, ColorImage, Context, Frame, Image, Key, Rgba,
        RichText, ScrollArea, TextEdit, ViewportBuilder, ViewportCommand, Visuals,
    },
...
impl EguiUi {
    ...
    fn scroll(&mut self, ctx: &Context) {
    ...
    fn get_icon(&mut self, application: &Application, ctx: &Context) -> Option<TextureId> {
...
impl eframe::App for EguiUi {
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        ...
        let execute = ctx.input(|i| i.key_pressed(Key::Enter));
        ...
        CentralPanel::default().show(ctx, |ui| {
            let response = ui.add(TextEdit::singleline(&mut self.search_str));
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (i, (application, _)) in (self.application_manager.matches.clone())
                        ...
                        .enumerate()
                    {
                        let label_text = RichText::new(application.name.clone());
                        let mut background_color = Color32::from_rgba_unmultiplied(0, 0, 0, 0);
                        if i == self.selected {
                            background_color = Color32::from_rgba_unmultiplied(0, 100, 0, 128);
                        }
                        let icon_texture_handle = self.get_icon(application, ctx);
                        let icon = match icon_texture_handle {
                            Some(handle) => handle,
                            None => {
                                if let Some(icon_handle) = self.placeholder_icon {
                                    icon_handle
                                } else {
                                    let icon_handle = ctx
                                        .load_texture(
